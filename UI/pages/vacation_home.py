...
class VacationHome:
    def __init__(self, db):
        super(VacationHome, self).__init__()
        self.__bs = None
        self.__table = None
        self.__crud = CRUD(db)
    ...
    def vacationhome(self, pg: PageData):
        def send_msg(e):
        ...
        def load_table_info(flag, restrictions=None):
            if not flag:
                ...
                data = self.__crud.get_restricted_query(restrictions, "Загородное_жильё")
            for row in data:
        ...
        pg.page.title = "Загородная недвижимость"
